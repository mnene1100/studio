      ...
      <div className="flex-1 overflow-y-auto px-4 pb-32 pt-4">
        {isChatsLoading ? (
          ...
          <div className="space-y-1">
            {sortedChats.map((chat) => (
              <Link 
                ...
                className="flex items-center px-4 py-4 rounded-[2rem] active:bg-white/5 transition-all group border border-transparent"
              >
                <div className="relative">
                  <Avatar className="w-14 h-14 rounded-[1.25rem] ring-2 ring-white/5 shadow-xl">
                    <AvatarFallback className="bg-muted text-lg font-black">
                      <MessageSquare className="w-6 h-6" />
                    </AvatarFallback>
                  </Avatar>
                  <div className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-primary rounded-full border-2 border-black" />
                </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center pb-20 px-10">
            <div className="w-32 h-32 bg-white/5 rounded-[2.5rem] flex items-center justify-center mb-8 border border-white/5 shadow-2xl">
            ...
            <button className="mt-6 px-8 py-3 bg-primary text-white rounded-full font-black uppercase tracking-widest text-[10px] shadow-2xl active:scale-95 transition-all">
              Start Chatting
            </button>
          </div>
        )}
